...
const ColorsTable = () => {
  ...
  const additionField = [
    {
      title: 'رنگ',
      elements: rowData => (
        <div className="w-100 h-100 d-block" style={{ background: rowData.code, color: rowData.code }}>
          ...
        </div>
      )
    },
  ...
  const handleDeleteColor = async color => {
    const resault = await Confirm("حذف برند",`آیا از حذف ${color.title} اطمینان دارید؟`)
