...
import { createNewProductService, editProductsService } from '../../services/products';
...
export const onSubmit = async (values, actions, productToEdit) => {
  ...
  if (productToEdit) {
    const res = await editProductsService(productToEdit.id, values);
    if (res.status == 200) {
...
export const validationSchema = Yup.object({
  ...
  // image: Yup.mixed()
    ...
    // .test('format', 'فرمت فایل باید jpg باشد', value => (!value ? true : value.type === 'image/jpeg')),
